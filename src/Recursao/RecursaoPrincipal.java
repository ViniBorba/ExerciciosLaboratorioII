...
public class RecursaoPrincipal {
	...
	//Exe 1 m�todo recursivo que imprime um array recebido por par�metro de tr�s para frente
	...
	//Exe 2 m�todo  recursivo  que  recebe  um  array  bidimensional  de  double  e retorna a soma dos elementos deste array
	public static double somaBidimensional(double[][] arrayBidi) {
		return somaBidimensional(arrayBidi, 0, 0);//vou te retornar esse m�todo que � um metodo que retorna um double
	}
	private static double somaBidimensional(double[][] arrayBidi, int linha, int coluna) {
		if(linha >= arrayBidi.length) {
			return 0;//quando n�o tiver mais linhas eu retorno zero	
		}
		if(coluna >= arrayBidi[linha].length) {
			return somaBidimensional(arrayBidi, linha + 1, 0);//quando terminar as colunas eu vou para a pr�xima linha
		}
	...
	//Exe 3 rie  um  m�todo  chamado  encontraPosicaoMenorValor.
	//encontrar  a  posi��o  do  menor  valor  de  um  array  de  inteiros  recebido  por par�metro
	...
	private static int encontraPosicaoMenorValor(int[] array, int indice, int menorValor) {
		...
		return encontraPosicaoMenorValor(array, indice +1, menorValor);//chama o metodo at� entrar no primeiro if
	}
	...
	public static void printPrimaryDiagonal  (double[][] arrayBidiDeInt) throws MatrixNotPossibleException, IllegalArgumentException{
		//verificar se o array n�o � null
		if(arrayBidiDeInt == null) {
			throw new IllegalArgumentException();
		}
		
		//verificar se o array � quadrado
